...
from pegasus_lidcidri_dataset import LidcidriData
...
def main(args):
    ...
    # ★ tracker を report_to に応じて初期化 ★
    if accelerator.is_main_process:
    # report_to が wandb の場合だけ tracker を初期化
        if "wandb" in (args.report_to or "").lower():
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler", revision=args.revision,local_files_only=True)
    image_encoder = CLIPVisionModelWithProjection.from_pretrained(args.pretrained_model_name_or_path, subfolder="image_encoder", revision=args.revision,local_files_only=True)
    ...
    unet = UNet2DConditionModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision,local_files_only=True)
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
           ...
           # raise ValueError("xformers is not available. Make sure it is installed correctly")
            print("[WARN] xformers not available → skipping memory efficient attention")
 ...
 # ======================
    ...
    patients_json_path = "/work/XRAYDIFF/share/lidcidri/img_simple_256/patients.json"

    train_dataset = LidcidriData(
        root_dir="/work/XRAYDIFF/share/lidcidri/img_simple_256",
        patients_json=patients_json_path,
        image_transforms=transform,
        validation=False,
    )

    validation_dataset = LidcidriData(
        root_dir="/work/XRAYDIFF/share/lidcidri/img_simple_256",
        patients_json=patients_json_path,
        image_transforms=transform,
        validation=True,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(optimizer):
                ...
                if accelerator.is_main_process:
                    ...
                    if train_log_dataloader is not None and (global_step % args.validation_steps == 0 or global_step == 1):
                      ...
                      #  )
                        ...
                        train_image_logs = log_validation(
                            validation_dataloader=train_log_dataloader,
                            vae=vae,
                            image_encoder=image_encoder,
                            feature_extractor=feature_extractor,
                            unet=unet,
